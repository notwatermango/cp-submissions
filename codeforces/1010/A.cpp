...
void solv(){
  ...
  double wnow = m;
  wnow = wnow/((double)1 - (double)1/b[0]);
  mango(wnow);
  for (int i = n-1; i > 0; --i)
  {
    wnow = wnow/((double)1 - (double)1/a[i]);
    wnow = wnow/((double)1 - (double)1/b[i]);
  }
  mango(wnow);
  wnow = wnow/((double)1 - (double)1/a[0]);
  mango(wnow);
  if(wnow > 1e18){
    cout << "-1\n";
    return;
  }
  cout <<fixed << setprecision(6)<< wnow-m << '\n';
  return;
